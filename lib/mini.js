    ...
    results() {
      ...
      const displaySorted = sorted.map((player, index) => {
        ...
        return '['+rank+'] <@'+player.id+'>: '+player.resp+' ('+player.gotVotes+' votes, '+Math.round(player.ratio*1000)/10+'%)'+
          (player.resp2 ? '    Response 2: '+player.resp2+' ('+player.gotVotes2+' votes, '+Math.round(player.ratio2*1000/10)+'%)');
      });
    ...
    generateImage() {
      ...
      this.players.forEach(player => {
        img.fill('#000')
           .fontSize(20)
           .stroke('#0000', 0)
           .drawText(PADDING + (this.round-0.5)*ROUND_WIDTH, PADDING + player.positions[players.positions.length-1]*PLAYER_HEIGHT, mainServer.members[player.id] ? 
