...
module.exports = function(mainServer, chat, localData, idFromName, bot, clever, DEMIPIXEL_ID) {
  ...
  function message(user, userID, channelID, message, pm) {
    if (message == 'create') {
      if (admins.indexOf(userID) == -1 && userID != DEMIPIXEL_ID && userID != queue[0] && queue.length) chat(channelID, '<@'+userID+'>: You cannot create Mini TWOWs!');
      ...
      else queue = queue.filter(id => id != userID);
    ...
    } else if (message == 'clever') {
      if (!game) chat(channelID, '<@'+userID+'>: No game is running yet!');
      else game.clever(userID, channelID);
    } else if (message == 'hyper') {
      if (!game) chat(channelID, '<@'+userID+'>: No game is running yet!');
      else game.hyper(userID, channelID);
    } else if (message.indexOf('allow') == 0) {
      if (!game) chat(channelID, '<@'+userID+'>: No game is running yet!');
      else if (!message.match(/allow .+/)) chat(channelID, '<@'+userID+'>: Correct format is `!mini allow <user>`');
      else game.allow(userID, message.match(/allow (.+)/)[1], channelID);
      ...
      for (var i = 0; i < this.round+1; i++) {
        this.players.forEach(player => {
          if (player.positions[i]) {
            img.stroke(player.color, 3)
               .fill(player.color)
               .drawPoint(PADDING + ROUND_WIDTH*i, PADDING + player.positions[i]*PLAYER_HEIGHT);
          }
          if (player.positions[i] != undefined && player.positions[i+1] != undefined) {
            img.stroke(player.color, 2)
               .fill(player.color)
               .drawLine(PADDING + ROUND_WIDTH*i, PADDING + player.positions[i]*PLAYER_HEIGHT,
                     PADDING + ROUND_WIDTH*(i+1), PADDING + player.positions[i+1]*PLAYER_HEIGHT)
          } else if (player.positions[i] == undefined && player.positions[i-1] != undefined) {
            img.fill('#000')
               .fontSize(20)
               .stroke('#000', 0)
               .drawText(PADDING + (this.round-0.5)*ROUND_WIDTH, PADDING + player.positions[i-1]*PLAYER_HEIGHT, mainServer.members[player.id] ? 
                                                                                        (mainServer.members[player.id].nick || 
                                                                                        mainServer.members[player.id].username) :
                                                                                        'Unknown Player');
          }
        });
      }
  ...
  class Player {
    ...
    giveVoteOptions() {
      ...
      this.voteOptions = {};
      const rand = this.game.getAlivePlayers(true).concat(this.game.getAlivePlayers(true).filter(player => !!player.resp2)); // Allow bot!
      shuffle(rand);
