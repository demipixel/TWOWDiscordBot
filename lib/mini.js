...
module.exports = function(mainServer, chat, localData, idFromName, bot, clever, DEMIPIXEL_ID) {
  ...
  class Mini {
    ...
    newRound(id, c, prompt) {
      ...
      if (this.cleverMode) {
        for (var i = 0; i < 2; i++) {
          var resp = i == 0 ? 'resp' : 'resp2';
          clever.ask(self.prompt, (err, resp) => {
            if (err) self.chat(err);
            else {
                ...
                clever.ask(self.prompt, (err, resp) => {
                  if (err) self.chat(err);
                    ...
                    if (resp.split(' ').length > 10) {
                      clever.ask(self.prompt, (err, resp) => {
                        if (err) self.chat(err);
                        else self.cleverBot[resp] = resp.split(' ').slice(0, 10).join(' ');
                      })
                    }
                    else self.cleverBot[resp] = resp;
                  }
                });
              }
              else self.cleverBot[resp] = resp;
      ...
      setTimeout(() => {
        console.log(self.cleverBot.resp,self.cleverBot.resp2);
      }, 5*1000);
    }
    ...
    results() {
      ...
      for (var i = 0; i < displaySorted.length; i++) {
        ...
        if ((i/size <= this.safe && !this.cleverBot) || (this.cleverMode && i < botPos)) aliveLeft++;
      }

      if (aliveLeft <= 1) {
        const winner = !sorted[0].isBot ? sorted[0] : sorted[1];
