...
module.exports = function(mainServer, chat, localData, idFromName, DEMIPIXEL_ID) {
    ...
    if (message == 'create') {
      ...
      if (!pm) chat(channelID, '<@'+userID+'>: This is a PM-Only command!');
        ...
        if (resp.length > 200) {
          ...
          game.getPlayer(userID).chat('Responsed saved. ('+resp.split(' ').length+' words)');
        }
      }
      ...
      chat('Mini TWOW Commands\n`join`: Join game\n`spec`: Join spectator\n`leave`: Leave game or spectator\n'+
        ...
        '`prompt <prompt>`: Give a prompt for a new round!\n`quit`: Quit being leader, randomly assigned to someone else.', id, c);
    }
  }
  ...
  class Mini {
    ...
    newRound(id, c, prompt) {
      ...
      setTimeout(() => { self.newVoting() }, 2*60*1000);
    }
    ...
    results() {
      var voteOptions = Math.min(this.getAlivePlayers().length, this.maxVoteOptions);
      this.players.concat(this.specs).forEach(user => {
        if (user.voteStr) {
          for (var i = 0; i < user.voteStr.length; i++) {
            const voted = user.voteOptions[user.voteStr[i]];
            voted.gotVotes++;
            voted.gotPoints += voteOptions - i;
          }
        }
      });
      this.getAlivePlayers().forEach(player => {
      ...
      const displaySorted = sorted.map((player, index) => {
        var rank = index + 1;
        return '['+rank+'] <@'+player.id+'>: '+player.resp+' ('+player.gotVotes+' votes, '+Math.round(player.ratio/(voteOptions+1)*1000)/1000+')';
      });
    ...
    newRandomCreator() {
      ...
      while (this.creator == oldCreator) {
        this.creator = Math.floor(Math.random()*active.length);
      }
    ...
    tellVote() {
      this.chat('Time to vote! Vote using `!mini vote <options>` e.g. `!mini vote ABCGFED`\n'+
                ...
                Object.keys(this.voteOptions).map(opt => {
                  var tooLong = this.voteOptions[opt].resp.split(' ').length > 10;
                  return opt.toUpperCase()+'. '+this.voteOptions[opt].resp+' ('+(tooLong?'**':'')+this.voteOptions[opt].resp.split(' ').length+' words'+(tooLong?'**':'')+')'
                }).join('\n'));
    }
