...
module.exports = function(mainServer, chat, localData, idFromName, bot, clever, DEMIPIXEL_ID) {
  ...
  function message(user, userID, channelID, message, pm) {
    ...
    if (cmd == 'create') {
      if (admins.indexOf(userID) == -1 && userID != DEMIPIXEL_ID && queue.length && userID != queue[0]) chat(channelID, '<@'+userID+'>: '+
      ...
      if (queue.indexOf(user) == -1) chat(channelID, '<@'+userID+'>: You are not in the queue.');
      else {
        queue = queue.filter(id => id != userID);
      ...
      if (!game) chat(channelID, '<@'+userID+'>: No game is running yet!');
      else if (userID != DEMIPIXEL_ID && admins.indexOf(userID) == -1) chat(channelID, '<@'+userID+'>: You don\'t have permission to do that!');
      else game.kick(userID, channelID);
    } else if (cmd == 'clever') {
      if (!game) chat(channelID, '<@'+userID+'>: No game is running yet!');
    ...
    } else if (cmd.indexOf('vote') == 0 && cmd != 'voteoff'/*'abnext'.indexOf(message.toLowerCase()) > -1 && pm*/) {
  ...
  class Mini {
    ...
    newRound(id, c, prompt) {
      ...
      this.getAlivePlayers(true).forEach((player, index) => {
        player.gotPoints = 0;
        player.gotVotes = 0;
        player.gotPoints2 = 0;
        player.gotVotes2 = 0;
    ...
    checkNotIdle(time, round) {
      ...
      if (this.round == round && !this.ended) {
        this.chat('Creator has been idle for too long.\n'+this.newRandomCreator());
      }
    }
    ...
    voteoff(id, c) {
      if (this.voteoffList.indexOf(id) > -1) this.chat('You have already voted!', id, c);
      else {
        this.voteoffList.push(id);
        if (this.voteoffList.length > Math.max(3, this.players.length/3)) {
    ...
    kick(id, c) {
      this.chat('The current leader was kicked.\n'+newRandomCreator());
    }
    ...
    newRandomCreator() {
      const active = this.getAlivePlayers().filter(p => p.timesLeader < 2);
      ...
      while (this.creator == oldCreator) {
        newLeader = active[Math.floor(Math.random()*active.length)]
        this.creator = newLeader.id;
      }
      newLeader.timesLeader++;
      this.checkNotIdle(2*60*1000);
      return 'New leader is <@'+this.creator+'>! Make a new prompt using `!mini prompt <Prompt here>`. You have 2 minutes to submit your prompt.';
    }
    ...
    endGame(force) {
      ...
      if (queue.length > 0) {
        const oldId = queue[0];
        if (!mainServer.members[oldId] || mainServer.members[oldId].status == 'offline') {
          this.chat('<@'+oldId+'> is offline, skipping his place in the queue.');
          queue.splice(0, 1);
          this.endGame(true);
        } else {
          this.chat('<@'+oldId+'>: It is now your turn to be the Mini TWOW creator! Use `!mini create`. You have 60 seconds!');
          const self = this;
          setTimeout(() => {
            if (queue.length && queue[0] == oldId) {
  ...
  class Player {
    ...
    vote(str) {
      ...
      if (!this.voteOptions) {
        this.chat('Error processing your vote...');
        this.giveVoteOptions();
        this.tellVote();
        return;
      }
      str = str.toLowerCase();
...
function countWords(str) {
  ...
  return str.replace(/[^a-zA-Z'-]/gi, ' ').split(' ').filter(word => !!word).length; // Not empty words
}
