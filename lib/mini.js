...
module.exports = function(mainServer, chat, localData, idFromName, DEMIPIXEL_ID) {
  ...
  function message(user, userID, channelID, message, pm) {
    if (message == 'create') {
    ...
    } else if (/*message.indexOf('vote') == 0 || */(message == 'a' || message == 'b') && pm) {
      ...
      chat(channelID, '<@'+userID+'>: Uknown command');
    }
  }
  ...
  class Mini {
    ...
    newRound(id, c, prompt) {
      ...
      const self = this;
      setTimeout(() => {
        const needToSubmit = this.getAlivePlayers().get
        self.chat('30 seconds remaining!');
      }, 2.5*60*1000);
      setTimeout(() => { self.newVoting() }, 3*60*1000);
    }
    ...
    newVoting() {
      ...
      'There are 3 minutes to vote! Get going!\n'+
    ...
    results() {
      ...
      const displaySorted = sorted.map((player, index) => {
        var rank = index + 1;
        return '['+rank+'] <@'+player.id+'>: '+player.resp+' ('+player.gotVotes+' votes, '+Math.round(player.ratio*1000)/1000+')';
      });
    ...
    joinSpec(id, c) {
      if (this.getPlayer(id, true)) this.chat('You are already spectating!', id, c);
        ...
        if (this.partA == false && this.started) {
          user.giveVoteOptions();
          user.tellVote();
        }
      }
  ...
  class Player {
    ...
    vote2(str) {
      str = str.toLowerCase();
      if (str != 'a' && str != 'b') { this.chat('You must vote with `A` or `B`.'); return; }
      const index = str == 'a' ? 0 : 1;
      this.voteOptions2[index].gotVotes++;
      this.voteOptions2[index].gotPoints++;

      this.voteOptions2[(index+1)%2].gotVotes++;

      this.voteOptions2.splice(0, 1);
      this.chat('Vote saved'+(this.totalVoteOptions2 - this.voteOptions2.length == 8?'\nYou have voted on 8 combinations.'+
                      ' You can continue if you wish, but you have reached the recommended number of votes.':''));
      if (this.voteOptions2.length > 0) tellVote2();
      this.votedLastTime = true;
    }

    giveVoteOptions() {
      return giveVoteOptions2();
      this.voteOptions = {};
    ...
    tellVote2() {
      ...
      else this.chat('Which is better? Respond with `A` or `B`.\n'+
