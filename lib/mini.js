...
module.exports = function(mainServer, chat, localData, idFromName, bot, clever, DEMIPIXEL_ID) {
  ...
  class Mini {
    ...
    generateImage() {
      ...
      // Draw usernames
      this.players.forEach(player => {
        if (!players.positions[this.round-1]) return;
        img.fill('#000')
           .fontSize(20)
           .stroke('#0000', 0)
           .drawText(PADDING + (this.round-0.5)*ROUND_WIDTH, PADDING + player.positions[this.round-1]*PLAYER_HEIGHT, mainServer.members[player.id] ? 
