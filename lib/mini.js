...
module.exports = function(mainServer, chat, localData, idFromName, bot, clever, DEMIPIXEL_ID) {
  ...
  function message(user, userID, channelID, message, pm) {
    ...
    if (cmd == 'create') {
      if (admins.indexOf(userID) == -1 && userID != DEMIPIXEL_ID && queue.length && userID != queue[0].id) chat(channelID, '<@'+userID+'>: '+
      ...
      else if (queue.indexOf(user) != -1) chat(channelID, '<@'+userID+'>: You are at position '+(queue.indexOf(user)+1)+' in the queue.');
    ...
    endGame(force) {
      ...
      if (queue.length > 0) {
        ...
        else
        {
          ...
          const self = this;
          setTimeout(() => {
            if (queue.length && queue[0].id == oldId) {
